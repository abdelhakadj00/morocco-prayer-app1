...
class _PrayerTimesScreenState extends State<PrayerTimesScreen> with SingleTickerProviderStateMixin {
  ...
  final List<Map<String, String>> allCities = [
    ...
    {'name': 'Sidi Kacem', 'ar': 'سيدي قاسم'},
    {'name': 'Sidi Slimane', 'ar': 'سيدي سليمان'},
    {'name': 'Mohammedia', 'ar': 'المحمدية'},
    {'name': 'Khenifra', 'ar': 'خنيفرة'},
    {'name': 'Azrou', 'ar': 'آزرو'},
    {'name': 'Ifrane', 'ar': 'إفران'},
    {'name': 'Midelt', 'ar': 'ميدلت'},
    {'name': 'Zagora', 'ar': 'زاكورة'},
    {'name': 'Tan-Tan', 'ar': 'طانطان'},
    {'name': 'Sidi Ifni', 'ar': 'سيدي إفني'},
    {'name': 'Tinghir', 'ar': 'تنغير'},
    {'name': 'Tarfaya', 'ar': 'طرفاية'},
    {'name': 'Smara', 'ar': 'السمارة'},
    {'name': 'Boujdour', 'ar': 'بوجدور'},
    {'name': 'Assa-Zag', 'ar': 'أسا الزاك'},
    {'name': 'Chefchaouen', 'ar': 'شفشاون'},
    {'name': 'Asilah', 'ar': 'أصيلة'},
    {'name': 'Martil', 'ar': 'مرتيل'},
    {'name': 'Fnideq', 'ar': 'الفنيدق'},
    {'name': 'Ouezzane', 'ar': 'وزان'},
    {'name': 'Sefrou', 'ar': 'صفرو'},
    {'name': 'Berkane', 'ar': 'بركان'},
    {'name': 'Figuig', 'ar': 'فكيك'},
    {'name': 'Jerada', 'ar': 'جرادة'},
    {'name': 'Boulemane', 'ar': 'بولمان'},
    {'name': 'Hajeb', 'ar': 'الحاجب'},
    {'name': 'Zag', 'ar': 'زاك'},
    {'name': 'Tata', 'ar': 'طاطا'},
    {'name': 'Aousserd', 'ar': 'أوسرد'},
  ];
  ...
  void _calculateNextPrayer() {
    ...
    for (var prayer in prayerOrder) {
      if (timings.containsKey(prayer) && timings[prayer]! > currentTime) {
        setState(() {
          nextPrayer = prayer;
        ...
        if (prayerTime.isAfter(now)) {
          ...
          if (notificationTime.isAfter(now)) {
            await notificationsPlugin.zonedSchedule(
              prayer.hashCode,
              'حان وقت صلاة $prayer',
              'متبقي 10 دقائق على صلاة $prayer',
              notificationTime,
            ...
            Row(
              ...
              children: [
                Expanded(
                  child: Column(
                    children: [
                      const Icon(Icons.calendar_today, color: Color(0xFF00695C), size: 32),
                      const SizedBox(height: 8),
                      const Text('التاريخ الهجري', style: TextStyle(color: Colors.grey, fontSize: 12)),
                      const SizedBox(height: 4),
                      Text(hijriDate, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.bold)),
                    ],
                  ),
                ),
                ...
                Expanded(
                  child: Column(
                    children: [
                      const Icon(Icons.calendar_month, color: Color(0xFF00695C), size: 32),
                      const SizedBox(height: 8),
                      const Text('التاريخ الميلادي', style: TextStyle(color: Colors.grey, fontSize: 12)),
                      const SizedBox(height: 4),
                      Text(gregorianDate, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.bold)),
                    ],
                  ),
                ),
              ],
  ...
  Future<void> _showQiblaDirection() async {
    final status = await PermissionHandler().requestPermissions([Permission.location]);
    
    if (status[Permission.location] != PermissionStatus.granted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('يجب منح إذن الموقع لعرض اتجاه القبلة')),
      );
      return;
    }
